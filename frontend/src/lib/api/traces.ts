...
export const tracesApi = {
  list: (params?: { runId?: number; status?: string; traceType?: string; agentConfigId?: number; limit?: number }) => {
  ...
  summary: (params?: { runId?: number; status?: string; traceType?: string; agentConfigId?: number }) => {
    const qs = new URLSearchParams();
    if (params?.runId !== undefined) qs.set("run_id", String(params.runId));
    if (params?.status) qs.set("status", params.status);
    if (params?.traceType) qs.set("trace_type", params.traceType);
